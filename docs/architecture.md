...
Provider-specific response objects stop at their adapter. A single provider factory resolves Gemini, OpenAI, or an explicitly injected deterministic test provider from deployment configuration. The Gemini adapter uses Google's current `@google/genai` SDK; the preserved OpenAI adapter uses the official OpenAI SDK. Domain logic receives validated RoleProwl types only and contains no provider-name branches.

Each allowed AI task owns a distinct versioned instruction, Zod schema, and optional model override. The OpenAI adapter uses Responses API structured parsing, while the Gemini adapter uses Gemini structured JSON output; both validate parsed values again at the RoleProwl boundary, detect refusals separately from malformed output, and return normalized correlation, provider-request, and token metadata. Only explicitly constructed task input is serialized. Logs contain operational metadata but exclude prompts, inputs, outputs, names, emails, and résumé content. Deterministic tests use a provider double and require no live API access.

RP-031A temporarily selects Gemini for synthetic-data manual-alpha testing. `gemini-3.5-flash-lite` handles routine extraction, normalization, classification, comparison, and ordinary writing. `gemini-3.5-flash` is reserved for résumé tailoring, cover letters, explicit difficult-task routing, or schema failure on an eligible Lite task after bounded retry. Rate-limit or quota failure never escalates models and never invokes OpenAI.

Gemini structured output uses a provider-local JSON Schema derived from the existing task Zod schema, followed by JSON decoding and the same Zod validation. Per-model daily and minute limits are consumed through the shared serializable PostgreSQL rate limiter immediately before every SDK request, including retries. The result exposes normalized provider/model/task/schema/latency/usage/retry/capacity metadata without logging content. Capacity failures use sanitized `LIMIT_REACHED`, `RATE_LIMITED`, or `PROVIDER_UNAVAILABLE` states.

Free-tier Gemini is synthetic-only. Authenticated UI displays a warning when selected, fixtures are explicitly fictional, preview initialization requires an explicit override, and public production initialization is refused by default. The restriction is enforced at provider initialization as well as communicated in the interface. OpenAI can later be restored with `AI_PROVIDER=openai` and its key without schema, database, feature, or policy changes.
...
RP-030 defines analytics as explicit product-lifecycle events, not passive behavioral surveillance. `AnalyticsProvider` accepts only the closed event vocabulary for discovery, viewing, shortlist/reject, application preparation/review/submission, and recorded responses/interviews/offers. The PostgreSQL adapter is the alpha provider. A unique dedupe key makes retrying instrumentation idempotent, while analytics failure is logged and cannot break the underlying product operation.

Each event stores an optional RoleProwl user ID, entity reference, occurrence time, and a per-event allowlisted property object. The allowlist excludes names, contact data, résumé or answer content, generated prose, URLs, IP addresses, device fingerprints, cookies, and provider payloads. Candidate-attributed events appear as aggregate counts in Settings, are included in account export, and cascade away with account deletion. Global job-discovery events have no user identity and retain only source type plus whether a new canonical job was created.

## Alpha settings surface

Settings exposes logical candidate controls without exposing provider secrets. Search preferences remain canonical Truth Vault data and link to their editor. Application authority is an actual persisted, validated policy editor; changing it writes a safe audit event and invalidates the dashboard threshold. Reusable answer memory shows concept, provenance, verification date, and whether auto-answering is allowed, while deliberately omitting answer values from the summary. Integration cards are projections of the central capability registry with all partner authorization set to false by default. Account export, deletion, analytics transparency, audit history, signed-in identity, and session-control guidance complete the privacy and account areas.

## Automated pre-alpha qualification

RP-031 exercises the major domain and use-case boundaries as ten synthetic candidate journeys. Controlled in-memory repositories, deterministic AI, fixture job feeds, private fake storage, and a fake authorized adapter make the success and failure paths reproducible without contacting an employer or external service. The journey suite is supplemented by the broader unit, integration, adapter-contract, safety-matrix, browser, schema, production-build, and dependency gates. Passing this gate means the source is coherent enough for user-led manual alpha testing; it is not evidence that credential-dependent integrations, a deployed network perimeter, or real employer systems have been validated.
